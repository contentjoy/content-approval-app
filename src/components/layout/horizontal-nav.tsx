          ...
          {tabs.map((t) => {
            ...
            return (
              <Link
                ...
                className={`no-underline hover:no-underline snap-start px-4 py-2 rounded-[999px] text-sm font-medium transition-all duration-200 cursor-pointer hover:bg-[var(--hover)] ${isActive ? 'text-[var(--foreground)] bg-transparent' : 'text-[var(--muted-text)] hover:text-[var(--foreground)]'}`}
              >
                {t.label}
              </Link>
            )
